...
def flag(request, comment_id, next=None):
    ...
    """
    comment = get_object_or_404(my_comment_app.get_model(), pk=comment_id)
...
@csrf_protect
@permission_required("my_comment_app.can_moderate")
def delete(request, comment_id, next=None):
    ...
    Context:
        comment
            the flagged `comments.comment` object
    """
    comment = get_object_or_404(my_comment_app.get_model(), pk=comment_id, site__pk=settings.SITE_ID)
...
def perform_flag(request, comment):
    ...
    """
    flag, created = my_comment_app.models.CommentFlag.objects.get_or_create(
        comment = comment,
        user    = request.user,
        flag    = my_comment_app.models.CommentFlag.SUGGEST_REMOVAL
    )
...
def perform_delete(request, comment):
    flag, created = my_comment_app.models.CommentFlag.objects.get_or_create(
        comment = comment,
        user    = request.user,
        flag    = my_comment_app.models.CommentFlag.MODERATOR_DELETION
    )
...
def perform_approve(request, comment):
    flag, created = my_comment_app.models.CommentFlag.objects.get_or_create(
        comment = comment,
        user    = request.user,
        flag    = my_comment_app.models.CommentFlag.MODERATOR_APPROVAL,
    )
